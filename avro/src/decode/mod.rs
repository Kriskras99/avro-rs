...
mod tests {
    use crate::{
        ...
        schema::{DecimalSchema, FixedSchema, InnerDecimalSchema, NamesRef, Schema, UuidSchema},
        types::{
            Value,
            Value::{Array, Int, Map},
        },
    ...
    fn decode(schema: &Schema, input: &[u8]) -> Result<Value, Error> {
        let names = NamesRef::default();
        let mut buffer = Buffer::from_slice(input);
        let fsm = SubFsm::new(schema, &names)?;
    ...
    fn test_decode_variable_overflow() {
        ...
        let res = decode_variable(causes_left_shift_overflow);
        assert!(res.is_err());
    }
