...
#[expect(
    ...
    reason = "Value is the import (and largest) variant"
)]
pub enum DataBlockFsmOutput<'a> {
    /// State machine read a value.
    Value((Value, DataBlockFsm<'a>)),
    /// State machine is finished.
    Finished(Header<'a>),
}

impl<'a> Fsm for DataBlockFsm<'a> {
    type Output = DataBlockFsmOutput<'a>;
    ...
    fn parse(mut self, buffer: &mut Buffer) -> FsmResult<Self, Self::Output> {
            ...
            match this.fsm.take().unwrap().parse(buffer)? {
                ...
                FsmControlFlow::Done((value, fsm)) => {
                    *this.left_in_block -= 1;
                    ...
                    Ok(FsmControlFlow::Done(Some((value, ()))))
                }
            }
        })?;
        ...
        match cf {
            ...
            FsmControlFlow::Done(Some((value, ()))) => Ok(FsmControlFlow::Done(
                DataBlockFsmOutput::Value((value, self)),
            )),
            FsmControlFlow::Done(None) => {
                let header = self.into_heads().header;
                Ok(FsmControlFlow::Done(DataBlockFsmOutput::Finished(header)))
            }
        }
    }
}
