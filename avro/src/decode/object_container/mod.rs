use crate::{
    Codec, Schema,
    decode::object_container::{
        data::{DataBlockFsm, DataBlockFsmOutput},
        header::HeaderFsm,
    },
    schema::NamesRef,
    types::Value,
...
pub enum Output<'a> {
    Value((Value, ObjectContainerFsm<'a, ReadingDataBlock>)),
    Finished(ObjectContainerFsm<'a, Finished>),
}
...
impl<'a> Fsm for ObjectContainerFsm<'a, ReadingDataBlock> {
    ...
    fn parse(mut self, buffer: &mut Buffer) -> FsmResult<Self, Self::Output> {
        ...
        match fsm.parse(buffer)? {
            ...
            FsmControlFlow::Done(DataBlockFsmOutput::Value((value, fsm))) => {
                self.inner = InnerObjectContainerFsm::DataBlock(fsm);
                Ok(FsmControlFlow::Done(Output::Value((value, self))))
            }
            FsmControlFlow::Done(DataBlockFsmOutput::Finished(header)) => {
                Ok(FsmControlFlow::Done(Output::Finished(ObjectContainerFsm {
                    state: PhantomData::<Finished>,
                    inner: InnerObjectContainerFsm::Finished(header),
                })))
            }
        }
    }
...
impl<'a> ObjectContainerFsm<'a, Finished> {
    /// Get the header of this file.
    pub fn header(&self) -> &Header<'_> {
        let InnerObjectContainerFsm::Finished(header) = &self.inner else {
            unreachable!()
        };
        header
    }

    /// Extract the file header from the finished state machine.
    pub fn into_header(self) -> Header<'a> {
        let InnerObjectContainerFsm::Finished(header) = self.inner else {
            unreachable!()
        };
