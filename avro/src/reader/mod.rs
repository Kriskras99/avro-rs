...
use crate::{
    ...
    decode::{
        DatumFsm,
        object_container::{Header, ObjectContainerFsm, Output, ReadingDataBlock},
    },
    error::Details,
    from_value,
    headers::{HeaderBuilder, RabinFingerprintHeader},
    schema::{AvroSchema, EMPTY_NAMES_REF, ResolvedOwnedSchema, ResolvedSchema, Schema},
    types::Value,
    ...
    fn read_next(&mut self) -> AvroResult<Option<Value>> {
        ...
        let mut item = loop {
            match fsm.parse(&mut self.buffer)? {
                ...
                FsmControlFlow::Done(Output::Value((value, fsm))) => {
                    self.fsm = FsmOrHeader::Fsm(fsm);
                    break value;
                }
                FsmControlFlow::Done(Output::Finished(fsm)) => {
                    self.fsm = FsmOrHeader::Header(fsm.into_header());
                    // Finished decoding
                    return Ok(None);
                }
            }
            fill_buffer(&mut self.reader, &mut self.buffer)?;
        };
...
pub fn from_avro_datum<R: Read>(
    ...
    let mut fsm = DatumFsm::new(writer_schema, &EMPTY_NAMES_REF)?;
    let value = loop {
        match fsm.parse(&mut buffer)? {
            FsmControlFlow::NeedMore(new_fsm) => {
                fsm = new_fsm;
